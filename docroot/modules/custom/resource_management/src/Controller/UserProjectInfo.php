...
class UserProjectInfo extends ControllerBase {
  ...
  /**
   * Displays the user project information.
   *
   * If user id is 0, then an empty autocomplete text box is displayed where
   * name of user must be entered.
   * If user id exists, then all nodes that contain that user's information
   * are displayed. Edit and Delete options are provided for editing and
   * deleting user info.
   */
  public function content($uId) {
    ...
    if (!empty($specialization)) {
      ...
      array_walk($specialization_vals, [$this, 'loadTerm']);
    ...
    $para_ids = $this->getUserBillingInformationParagraphs($uId);
  ...
  /**
   * Deletes particular paragraph of user.
   */
  ...
  /**
   * Loads specification of user.
   */
  private function loadTerm(&$val, $key) {
    $term = Term::load($val);
    $val = $term->get('name')->getValue()[0]['value'];
  }

  /**
   * Gets billing information of user.
   */
  private function getUserBillingInformationParagraphs($uId) {
